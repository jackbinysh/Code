...
%Trapezium
p1 = polyfit(log(TrapeziumResults(2,:)),log(abs(TrapeziumResults(3,:) - TrapeziumResults(4,:))),1);
syms x;
f1 = p1(1)*x + p1(2);
...
hold on
ezplot(f1);
plot(log(TrapeziumResults(2,:)),log(abs(TrapeziumResults(3,:) - TrapeziumResults(4,:))),'x');
hold off

%Simpsons
p2 = polyfit(log(SimpsonsResults(2,:)),log(abs(SimpsonsResults(3,:) - SimpsonsResults(4,:))),1);
syms y;
f2 = p2(1)*y + p2(2);
...
hold on
ezplot(f2);
plot(log(SimpsonsResults(2,:)),log(abs(SimpsonsResults(3,:) - SimpsonsResults(4,:))),'x');
hold off
